...
public class FuncEndpoint {
    ...
    public @ResponseBody
    ResponseDto filter(@RequestParam(value = "status", required = false) Integer status) {
        ResponseDto responseDto  = new ResponseDto();
        try {
            if (status == null) {
                List<FuncDto> funcDtos = funcService.findAll();
    ...
    public @ResponseBody
    ResponseDto create(@RequestParam(value = "name") String name,
    ...
    public @ResponseBody
    ResponseDto update(@RequestParam(value = "id") Long id,
