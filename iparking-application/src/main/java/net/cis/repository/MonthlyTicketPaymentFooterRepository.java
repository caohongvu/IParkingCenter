...
public interface MonthlyTicketPaymentFooterRepository extends JpaRepository<MonthlyTicketPaymentFooterEntity, Long> {
	@Query("SELECT ticket FROM MonthlyTicketPaymentEntity WHERE (ticket.transaction_id LIKE CONCAT('%',:transID,'%') "
			+ "OR :transID is NULL) "
	...
	public MonthlyTicketPaymentFooterEntity findAll( @Param("transID") String transID, @Param("cpp_code") String cpp_code,
